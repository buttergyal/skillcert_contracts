use soroban_sdk::{Address, String, Env, Vec, testutils::Address as _};
use crate::schema::{LightProfile, UserRole, UserStatus, DataKey};
...
fn setup_test_users(env: &Env, contract_id: &Address) -> (Address, Vec<Address>, Vec<LightProfile>) {
    let client = UserManagementClient::new(env, contract_id);
    ...
    // Create test users
    let test_data = [
        ("Alice Johnson", "US", Some("Developer"), UserRole::Student, UserStatus::Active),
        ("Bob Smith", "UK", Some("Teacher"), UserRole::Instructor, UserStatus::Active),
        ("Charlie Brown", "US", None, UserRole::Admin, UserStatus::Active),
        ("Diana Ross", "Canada", Some("Designer"), UserRole::Student, UserStatus::Inactive),
        ("Eve Wilson", "UK", Some("Manager"), UserRole::Instructor, UserStatus::Suspended),
    ];
    
    for (name, country, profession, role, status) in test_data.iter() {
        let (user_addr, profile) = create_light_profile(env, name, country, *profession, role.clone(), status.clone());
        users.push_back(user_addr.clone());
        profiles.push_back(profile.clone());
        
        // Store profile in contract storage context
        env.as_contract(contract_id, || {
            env.storage().persistent().set(&DataKey::UserProfileLight(user_addr), &profile);
        });
    }

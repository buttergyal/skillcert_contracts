...
use crate::schema::{UserProfile, DataKey, LightProfile, UserRole, UserStatus};
...
pub fn user_management_save_profile(
    ...
    env.storage().persistent().set(&light_storage_key, &light_profile);
...
fn add_to_users_index(env: Env, user: Address) {
    let mut users_index: Vec<Address> = env.storage().persistent()
    ...
    if !users_index.iter().any(|u| u == user) {
        ...
        env.storage().persistent().set(&DataKey::UsersIndex, &users_index);
    }
}

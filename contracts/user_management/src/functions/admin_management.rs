...
pub fn initialize_system(
    ...
    if let Some(existing_config) = env.storage().persistent().get::<DataKey, AdminConfig>(&DataKey::AdminConfig) {
    ...
    env.storage().persistent().set(&DataKey::Admins, &empty_admins);
...
pub fn add_admin(env: Env, caller: Address, new_admin: Address) {
    ...
    let config = env.storage().persistent().get::<DataKey, AdminConfig>(&DataKey::AdminConfig)
    ...
    let mut admins: Vec<Address> = env.storage().persistent().get::<DataKey, Vec<Address>>(&DataKey::Admins)
...
pub fn remove_admin(env: Env, caller: Address, admin_to_remove: Address) {
    ...
    let admins: Vec<Address> = env.storage().persistent().get::<DataKey, Vec<Address>>(&DataKey::Admins)
        .unwrap_or_else(|| Vec::new(&env));
    ...
    env.storage().persistent().set(&DataKey::Admins, &new_admins);
}
...
pub fn get_admins(env: Env, caller: Address) -> Vec<Address> {
    ...
    let regular_admins: Vec<Address> = env.storage().persistent().get::<DataKey, Vec<Address>>(&DataKey::Admins)
        .unwrap_or_else(|| Vec::new(&env));

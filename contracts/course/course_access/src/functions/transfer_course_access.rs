...
pub fn transfer_course_access(
    ...
    env.storage().persistent().set(&DataKey::CourseAccess(course_id.clone(), to.clone()), &course_access);

    // Remove the old user's access
    env.storage().persistent().remove(&key);

    // Extend the TTL for the new user's storage entry
    env.storage().persistent().extend_ttl(&DataKey::CourseAccess(course_id.clone(), to.clone()), 100, 1000);


    //emit an event 
     env.events().publish((COURSE_TRANSFER_EVENT,), (course_id, from, to));
